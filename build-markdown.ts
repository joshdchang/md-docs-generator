...
function generateTocHtml(headings: Heading[]): string {
  return headings.map(h => {
    ...
    const indent = h.depth > 2 ? `<div class="navIndent" style="left: ${(h.depth - 2) * 14 - 5}px"></div>` : '';
    return `
      <li class="navItem">
...
function generateHtmlPage(markdownHtml: string, tocHtml: string, config: any = {}): string {
...
// Main build function
export async function buildMarkdown(markdownPath: string, outputPath: string, config?: any) {
  try {
    // Read markdown file
    ...
    const html = generateHtmlPage(markdownHtml, tocHtml, config);
    
    // Write output
    fs.writeFileSync(outputPath, html);
    
    console.log(`Built ${markdownPath} -> ${outputPath}`);
  } catch (error) {
...
if (require.main === module) {
  ...
  if (!input || !output) {
    console.error('Usage: ts-node build-markdown.ts <input.md> <output.html>');
    process.exit(1);
  }
  
  buildMarkdown(input, output).catch(err => {
    console.error(err);
    process.exit(1);
  });
}